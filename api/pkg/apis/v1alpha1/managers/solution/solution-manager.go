...
func (s *SolutionManager) AsyncReconcile(ctx context.Context, deployment model.DeploymentSpec, remove bool, namespace string, targetName string) (model.SummarySpec, error) {
	...
	err = s.CheckJobId(ctx, deployment.JobID, namespace, deployment.Instance.ObjectMeta.GetSummaryId(), deployment.Instance.ObjectMeta.Name)
	if err != nil {
	...
	s.VendorContext.Publish(model.DeploymentPlanTopic, v1alpha2.Event{
		...
		Body: model.PlanEnvelope{
			...
			PlanId:               summaryId,
			PlanName:             deployment.Instance.ObjectMeta.Name,
			Remove:               remove,
			Namespace:            namespace,
...
func (s *SolutionManager) HandleDeploymentPlan(ctx context.Context, event v1alpha2.Event) error {
	...
	lockName := api_utils.GenerateKeyLockName(summary.PlanState.Namespace, planEnvelope.PlanId)
	tryLockresult := s.KeyLockProvider.TryLock(lockName)
	log.InfofCtx(ctx, "M (Solution): Try lock result %s", tryLockresult)
	err = s.CheckJobId(ctx, summary.PlanState.Deployment.JobID, summary.PlanState.Namespace, planEnvelope.PlanId, summary.PlanState.Deployment.Instance.ObjectMeta.Name)
	if err != nil {
		s.KeyLockProvider.UnLock(lockName)
		return err
	}
...
func (s *SolutionManager) HandleDeploymentStep(ctx context.Context, event v1alpha2.Event) error {
	...
	tryLockresult := s.KeyLockProvider.TryLock(lockName)
	log.InfofCtx(ctx, "M (Solution): Try lock result %s", tryLockresult)
	err = s.CheckJobId(ctx, stepEnvelope.PlanState.Deployment.JobID, stepEnvelope.PlanState.Namespace, stepEnvelope.PlanState.Deployment.Instance.ObjectMeta.GetSummaryId(), stepEnvelope.PlanState.Deployment.Instance.ObjectMeta.Name)
	if err != nil {
		s.KeyLockProvider.UnLock(lockName)
		return err
	}
	...
	err = s.CheckJobId(ctx, planState.Deployment.JobID, planState.Namespace, stepEnvelope.PlanState.Deployment.Instance.ObjectMeta.GetSummaryId(), planState.Deployment.Instance.ObjectMeta.Name)
	if err != nil {
...
func (s *SolutionManager) HandleStepResult(ctx context.Context, event v1alpha2.Event) error {
	...
	tryLockresult := s.KeyLockProvider.TryLock(lockName)
	log.InfofCtx(ctx, "M (Solution): Try lock result %s", tryLockresult)
	err = s.CheckJobId(ctx, planState.Deployment.JobID, planState.Namespace, planState.Deployment.Instance.ObjectMeta.GetSummaryId(), planState.Deployment.Instance.ObjectMeta.Name)
	if err != nil {
		s.KeyLockProvider.UnLock(lockName)
		return err
	}
...
func (s *SolutionManager) CheckJobId(ctx context.Context, jobID string, namespace string, summaryId string, summaryName string) error {
	ctx, span := observability.StartSpan("Solution Manager", ctx, &map[string]string{
		"method": "CheckJobId",
	})
	var err error = nil
	defer observ_utils.CloseSpanWithError(span, &err)
	defer observ_utils.EmitUserDiagnosticsLogs(ctx, &err)
	oldSummary, err := s.GetSummary(ctx, summaryId, summaryName, namespace)
