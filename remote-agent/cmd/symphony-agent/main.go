...
func main() {
	log.Printf("Symphony Agent started, args: %v", os.Args)
	...
	flag.StringVar(&logDir, "log-dir", "", "Directory for log files (default: current working directory)")
	flag.BoolVar(&useCertSubject, "use-cert-subject", false, "Use certificate subject as topic suffix instead of target name")
	flag.Parse()

	// Determine log file path based on --log-dir parameter
	...
	if logDir != "" {
		...
		agentLogPath = filepath.Join(logDir, "remote-agent.log")
	} else {
		agentLogPath = "remote-agent.log"
	}

	// Set up log file
	logFile, err := os.OpenFile(agentLogPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
	...
	log.Printf("=== Agent Execution Command ===")
	...
	log.Printf("Log File: %s", agentLogPath)
	log.Printf("===============================")
	...
	if providers == nil {
		log.Fatalf("failed to compose target providers")
	}
	...
	if protocol == "http" {
		if config.RequestEndpoint == "" || config.ResponseEndpoint == "" || config.BaseUrl == "" {
			log.Fatalf("RequestEndpoint, ResponseEndpoint, and BaseUrl must be set in the configuration file")
		}
		...
		if caPath != "" {
			...
			if err != nil {
				log.Fatalf("failed to read Symphony server CA certificate: %v", err)
			}

			serverCACertPool := x509.NewCertPool()
			if !serverCACertPool.AppendCertsFromPEM(serverCACert) {
				log.Fatalf("failed to parse Symphony server CA certificate")
			}
		...
		if err != nil {
			log.Fatalf("failed to update topology: %v", err)
		}
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			...
			log.Fatalf("topology update failed with status %d: %s", resp.StatusCode, string(bodyBytes))
		}

		log.Printf("Topology updated successfully via HTTP")

		if err := h.Launch(); err != nil {
			log.Fatalf("error launching HttpBinding: %v", err)
		}
		select {}
	} else {
		...
		if err != nil {
			log.Fatalf("failed to read MQTT CA file: %v", err)
		}
		if ok := caCertPool.AppendCertsFromPEM(caCert); !ok {
			log.Fatalf("failed to append MQTT CA cert")
		}
		...
		if brokerAddr == "" {
			...
			if brokerAddr == "" {
				log.Fatalf("MQTT broker address is required")
			}
		}
		...
		if err := m.Subscribe(); err != nil {
			log.Fatalf("failed to setup MQTT subscription: %v", err)
		}
		...
		if err := m.UpdateTopology(topologyContent); err != nil {
			log.Fatalf("topology update failed: %v", err)
		}
		log.Printf("Topology update confirmed successful")
		if err := m.Launch(); err != nil {
			log.Fatalf("failed to launch MQTT binding: %v", err)
		}
